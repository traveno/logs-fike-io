<script lang="ts">
  ...
  interface Props {
    posts: Post[];
    heading: string;
    /** When set, only the first `limit` posts are shown. */
    limit?: number;
  }

  let { posts, heading, limit }: Props = $props();
  let displayedPosts = $derived(limit ? posts.slice(0, limit) : posts);
</script>

<div class="flex flex-col gap-6">
  <h1 class="text-3xl font-mono font-thin">{heading}</h1>
  ...
  {:else}
    <div class="grid gap-4 lg:grid-cols-2">
      {#each displayedPosts as post}
        <a href="/{post.slug}">
          <div
            class="border border-neutral-300 p-4 rounded-lg flex flex-col justify-between transition text-neutral-700 hover:shadow-lg hover:scale-[1.01]"
          >
